	...
	$scope.importData = function(){
		...
		if( selectedTemp != "" )
		{
			if( selectedTemp.typeId == 1 )
			{
				if( selectedTemp.columnMetaData == "o" )
				{
					...
					// for push orgUnitUIDS

					for( var k = selectedTemp.columnStart.cn ; k <= selectedTemp.columnEnd.cn ; k++ )
					{
						var orGUnitLaval = $scope.getImportData( selectedTemp.columnStart.rn , k );
						orgUnitUIDs.push(  $scope.getOrgUnitByLabel( orGUnitLaval ) );
					}
				...
				{					
					...
					for( var i = selectedTemp.rowStart.rn ; i <= selectedTemp.rowEnd.rn ; i++ )
					{
						var orGUnitLaval = $scope.getImportData( i, selectedTemp.rowStart.cn );
						orgUnitUIDs.push(  $scope.getOrgUnitByLabel( orGUnitLaval ) );
					}
			...
			else
			{
				...
				//{
				  ...
				  orgUnitUIDs.push( $scope.getOrgUnitByLabel( $scope.getImportData( selectedTemp.orgUnitCell.rn , selectedTemp.orgUnitCell.cn ) ) );
					for( var x = 0 ; x < selectedTemp.DEMappings.length ; x++ )
					{
		...
		$.get("../api/system/info", function(data){
			...
			ExcelMappingService.importData( dataValueSet ).then(function(tem){
				...
				{
					for ( var i=0; i<orgUnitUIDs.length;i++)
					{
						...
						console.log( orgUnitUIDs[i] + " --" +  $("#imDataSetId").val() + "--" +  $("#importPeriod").val() );
					}
